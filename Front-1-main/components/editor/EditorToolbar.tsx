...
import React, { useEffect, useMemo, useState, useRef, useContext, ReactNode } from "react";
import ReactQuill, { Quill} from "react-quill";
import "react-quill/dist/quill.snow.css";
import "./custom-quill.css";
import { paraphrase, summarize,advanced_check,pdf,complete} from "@/app/lib/helpers";
import { Import_editor, savedocs, uplo, upload} from "@/app/api/search_utils/literature_utils";
...
console.log(download_url);
  ...
  return {success:true};} catch (error) {return {success:false};}
};}
const saveToFile = async (name:string) => {
  ...
  if (content?.innerHTML) {
    if (content.innerHTML != '<p><br></p>' ) {
     ...
     else {content.innerHTML = 'you can not save an empty doucument'
      return {success:false}
     }
  }
...
useEffect(() => {
 ...
 return(
  <div id="toolbar">
      ...
      {showModal && 
 <Savemodal isOpen={showModal} onClose={() => setShowModal(false)}>
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
    ...
    <button onClick={handlecancel}>Cancel</button></div> </section>
 </Savemodal>} 
      {showModal2 && <Savemodal isOpen={showModal2} onClose={() => setShowModal2(false)}>
      <section className="import">
        <h1>Saved works</h1>
        <div>
      {options.map((option, index) => (
        <button key={index} onClick={() => handleOptionSelect(option.name)}>
          {option.name}
        </button>
      ))}
      ...
      </section> <button onClick={handlecancel2}>Cancel</button> 
        </Savemodal>}
        {showModal3 && 
 <Savemodal isOpen={showModal3} onClose={() => setShowModal3(false)}>
  <section className="imp">
  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
    <input
      type="text"
    ...
    <button onClick={handlecancel3}>Cancel</button></div> </section>
    </Savemodal>} 
